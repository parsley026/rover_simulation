...
public:
    ...
    explicit KinematicsEstimator(const KinematicsConfig& config = KinematicsConfig{});
...
private:
    ...
    std::array<WheelVelocity, 4> extractWheelVectors(const rex_interfaces::msg::Wheels& feedback) const;
    
    SolverResult solveBodyTwist(const std::array<WheelVelocity, 4>& wheel_vectors);
    
    void updateCovariance(const SolverResult& result, double dt);

    // void integratePose(double v_x, double v_y, double omega, double dt);

    void integratePose(double velocity_x_mps, double velocity_y_mps, double angular_velocity_rad_s, double dt_s);
    ...
    public:
        WheelResidual(const double* v_xi_ptr, const double* v_yi_ptr, const double* weight_ptr, double x_i, double y_i)
            : v_xi_ptr_(v_xi_ptr), v_yi_ptr_(v_yi_ptr), weight_ptr_(weight_ptr), x_i_(x_i), y_i_(y_i) {}
    ...
    std::array<double, 9> pose_covariance_{{
        0.001, 0.0,   0.0,
        0.0,   0.001, 0.0,
        0.0,   0.0,   0.001
    }};
