 ...
 * @brief Convert between physical units (m, rad, m/s) and hardware units (ERPM, degrees, precise_pos).
 *
 * @details
 ...
 * - Input/outputs to the public API use physical units: meters (m), radians (rad), meters/sec (m/s).
 ...
 */
     ...
     * set_value = (mps * 30 / (pi * wheel_radius)) * poles_pairs_number * motor_gear_ratio + sign_offset
     *
     * @param mps Linear speed in meters/sec.
     * @param wheel_index Index of the wheel (0..3) used to apply polarity.
     * @return Hardware set value corresponding to the requested speed (ERPM units).
     */
    ...
    /**
     * @brief Convert hardware precise_pos to steering set value while applying polarity.
     *
     ...
     */
    double steeringSetFromPrecisePos(double precise_pos, std::size_t wheel_index) const;

    /**
     * @brief Map VESC ID from CAN messages to the drive wheel index used by the stack.
     * @param vesc_id Raw VESC ID from incoming messages.
     * @return Wheel index in [0..3] corresponding to front_left, front_right, rear_left, rear_right.
     */
    static std::size_t vescIdToDriveWheelIndex(uint8_t vesc_id);

    /**
     * @brief Map VESC ID from CAN messages to the steering wheel index used by the stack.
     * @param vesc_id Raw VESC ID from incoming messages.
