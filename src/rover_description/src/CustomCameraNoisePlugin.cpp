...
class CustomCameraNoisePlugin : public gz::sim::System,
...
public:
  void Configure(const gz::sim::Entity & /*_entity*/,
    ...
    if (_sdf->HasElement("topic")) {
      ...
      gzerr << "[CustomCameraNoisePlugin] <topic> not specified in plugin! "
               "Cannot apply noise."
            << std::endl;
      return;
    }
    ...
    gzmsg << "[CustomCameraNoisePlugin] Configured with param1=" << this->param1
          << ", param2=" << this->param2 << std::endl;
    gzmsg << "[CustomCameraNoisePlugin] Listening to: " << cameraTopic
          << std::endl;
    gzmsg << "[CustomCameraNoisePlugin] Publishing to: " << noisyTopic
          << std::endl;
  }
...
private:
  void OnImage(const gz::msgs::Image &_msg) {
    ...
    if (channels > 0) {
      ...
      if (!warned) {
        gzerr << "[CustomCameraNoisePlugin] FORMAT MISMATCH! Received format "
                 "type: "
...
GZ_ADD_PLUGIN(rover_description::CustomCameraNoisePlugin, gz::sim::System,
              rover_description::CustomCameraNoisePlugin::ISystemConfigure)
GZ_ADD_PLUGIN_ALIAS(rover_description::CustomCameraNoisePlugin,
