  ...
  if (max_wheel_speed_m_s > config_.max_wheel_speed_mps_) {
    for (int wheel_index = 0; wheel_index < 4; ++wheel_index) {
      command.drive_velocity_mps[wheel_index] =
          command.drive_velocity_mps[wheel_index] *
          (config_.max_wheel_speed_mps_ / max_wheel_speed_m_s);
    }
  }
}
...
void KinematicsSolver::enforceMinimumTurningRadius(
  ...
  if (turning_radius < config_.min_steering_radius_) {
    velocity_omega_rad_s =
        std::copysign(std::fabs(velocity_x_mps) / config_.min_steering_radius_,
                      velocity_omega_rad_s);
  }
}
...
KinematicsSolver::computeSymmetricAckermann(double velocity_x_mps,
  ...
  ackermann_limit_rad =
      std::atan2(1.0 * config_.wheelbase_,
...
KinematicsSolver::computeFrontOnlyAckermann(double velocity_x_mps,
  ...
  double ackermann_limit_rad;
  ackermann_limit_rad =
      std::atan2(2.0 * config_.wheelbase_,
                 (2.0 * config_.min_steering_radius_) - config_.track_width_);
