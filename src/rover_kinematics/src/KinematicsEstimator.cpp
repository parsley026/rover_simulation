...
void KinematicsEstimator::setConfig(const KinematicsConfig& config) {
    config_ = config;
    pose_covariance_ = {
        config_.pose_covariance_diagonal_[0], 0.0, 0.0,
        0.0, config_.pose_covariance_diagonal_[1], 0.0,
        0.0, 0.0, config_.pose_covariance_diagonal_[5]
    };
...
void KinematicsEstimator::setWheelQuality(const std::array<double, 4>& wheel_quality) {
    for (std::size_t wheel_index = 0; wheel_index < wheel_quality.size(); ++wheel_index) {
        const double clamped_quality = std::clamp(wheel_quality[wheel_index], config_.wheel_quality_min_weight_, 1.0);
        wheel_quality_[wheel_index] = config_.enable_dynamic_wheel_weighting_ ? clamped_quality : 1.0;
    }
}
...
void KinematicsEstimator::initOdomMessage() {
    ...
    odometry_.header.frame_id = config_.odom_frame_id_;
    odometry_.child_frame_id  = config_.base_frame_id_;
    ...
    odometry_.pose.covariance = {
        pose_covariance_[0], pose_covariance_[1], 0., 0., 0., pose_covariance_[2],
        pose_covariance_[3], pose_covariance_[4], 0., 0., 0., pose_covariance_[5],
        0., 0., config_.pose_covariance_diagonal_[2], 0., 0., 0.,
        0., 0., 0., config_.pose_covariance_diagonal_[3], 0., 0.,
        0., 0., 0., 0., config_.pose_covariance_diagonal_[4], 0.,
        pose_covariance_[6], pose_covariance_[7], 0., 0., 0., pose_covariance_[8]
    };
    ...
    odometry_.twist.covariance = {
        config_.twist_covariance_diagonal_[0], 0., 0., 0., 0., 0.,
        0., config_.twist_covariance_diagonal_[1], 0., 0., 0., 0.,
        0., 0., config_.twist_covariance_diagonal_[2], 0., 0., 0.,
        0., 0., 0., config_.twist_covariance_diagonal_[3], 0., 0.,
        0., 0., 0., 0., config_.twist_covariance_diagonal_[4], 0.,
        0., 0., 0., 0., 0., config_.twist_covariance_diagonal_[5]
    };
}
...
void KinematicsEstimator::initTransformMessage() {
    ...
    transformation_.transforms[0].header.frame_id = config_.odom_frame_id_;
    transformation_.transforms[0].child_frame_id  = config_.base_frame_id_;
...
OdometryEstimate KinematicsEstimator::update(const rex_interfaces::msg::Wheels& feedback, const rclcpp::Time& timestamp) {
    ...
    if (dt <= 0.0 || dt > config_.max_allowed_dt_) {
        return estimate; 
    }
    ...
    if (!result.is_usable) {
        ...
        if (warning_counter++ % 20 == 0) { // Throttle warning to avoid spamming the log
            RCLCPP_WARN(rclcpp::get_logger("KinematicsEstimator"), 
                        "Ceres solver failed! Falling back to dead reckoning.");
        }
                             
        integratePose(guess_velocity_x_mps_, guess_velocity_y_mps_, guess_velocity_omega_rad_s_, dt);
        return populateRosMessages(timestamp);
    } else {
        // Reset the counter once the solver recovers
        static int warning_counter = 0; 
        warning_counter = 0;
    }
    ...
    integratePose(result.velocity_x_mps, result.velocity_y_mps, result.velocity_omega_rad_s, dt);
...
double KinematicsEstimator::updateAndGetTimeDelta(const rclcpp::Time& timestamp) {
    ...
    const rex_interfaces::msg::Wheels& feedback) const 
{
    ...
    return {
        wheelVel(feedback.front_left.drive.set_value,  degreesToRadians(feedback.front_left.turn.set_value)),
        wheelVel(feedback.front_right.drive.set_value, degreesToRadians(feedback.front_right.turn.set_value)),
        wheelVel(feedback.rear_left.drive.set_value,   degreesToRadians(feedback.rear_left.turn.set_value)),
        wheelVel(feedback.rear_right.drive.set_value,  degreesToRadians(feedback.rear_right.turn.set_value))
    };
}
    ...
    const std::array<WheelVelocity, 4>& wheel_vectors) 
{
    if (!problem_initialized_) {
        // Destroy the old problem (and safely auto-delete all old Ceres-owned pointers)
        ...
        const double wheel_y[4] = { half_tw, -half_tw,  half_tw, -half_tw }; // FL is positive Y, FR is negative Y

        // Elegantly create and assign residuals inside a loop
        for (int i = 0; i < 4; ++i) {
            auto* residual = new WheelResidual(&measured_vx_[i], &measured_vy_[i], &wheel_quality_[i], wheel_x[i], wheel_y[i]);
            auto* cost_function = new ceres::AutoDiffCostFunction<WheelResidual, 2, 1, 1, 1>(residual);
            
            // Ceres takes ownership of cost_function and the HuberLoss pointer
            problem_->AddResidualBlock(
                cost_function,
                new ceres::HuberLoss(config_.huber_loss_threshold_),
                &solver_velocity_x_mps_,
                &solver_velocity_y_mps_,
    ...
    options.max_num_iterations = config_.solver_max_num_iterations_;
    options.function_tolerance = config_.solver_function_tolerance_;
    options.gradient_tolerance = config_.solver_gradient_tolerance_;
    options.parameter_tolerance = config_.solver_parameter_tolerance_;
    options.linear_solver_type = config_.solver_linear_solver_type_;
    options.trust_region_strategy_type = config_.solver_trust_region_strategy_type_;
    options.minimizer_progress_to_stdout = config_.solver_minimizer_progress_to_stdout_;
...
void KinematicsEstimator::updateCovariance(const SolverResult& result, double dt) {
    const int num_residuals = 8;
    const int num_params = 3;
    double sigma2 = (num_residuals - num_params > 0) 
        ? (2.0 * result.final_cost) / static_cast<double>(num_residuals - num_params)
        : 1e-6;
    ...
    if (covariance.Compute(cov_blocks, problem_.get())) {
        ...
        const double cov_vx_vx = getCov(&solver_velocity_x_mps_, &solver_velocity_x_mps_);
        const double cov_vx_vy = getCov(&solver_velocity_x_mps_, &solver_velocity_y_mps_);
        const double cov_vx_om = getCov(&solver_velocity_x_mps_, &solver_velocity_omega_rad_s_);
        const double cov_vy_vy = getCov(&solver_velocity_y_mps_, &solver_velocity_y_mps_);
        const double cov_vy_om = getCov(&solver_velocity_y_mps_, &solver_velocity_omega_rad_s_);
        const double cov_om_om = getCov(&solver_velocity_omega_rad_s_, &solver_velocity_omega_rad_s_);
        ...
        if (std::abs(angular_velocity_rad_s) < config_.se2_integration_omega_threshold_) {
            ...
            delta_x_local_m = (velocity_x_mps * sin_wdt + velocity_y_mps * (cos_wdt - 1.0)) / angular_velocity_rad_s;
            delta_y_local_m = (velocity_x_mps * (1.0 - cos_wdt) + velocity_y_mps * sin_wdt) / angular_velocity_rad_s;
            ...
            const double dnx_d_om = dt * (velocity_x_mps * cos_wdt - velocity_y_mps * sin_wdt);
            const double dny_d_om = dt * (velocity_x_mps * sin_wdt + velocity_y_mps * cos_wdt);

            const double nx = velocity_x_mps * sin_wdt + velocity_y_mps * (cos_wdt - 1.0);
            const double ny = velocity_x_mps * (1.0 - cos_wdt) + velocity_y_mps * sin_wdt;
        ...
        const double g[9] = {
            cos_yaw * d_dx_d_vx - sin_yaw * d_dy_d_vx, cos_yaw * d_dx_d_vy - sin_yaw * d_dy_d_vy, cos_yaw * d_dx_d_om - sin_yaw * d_dy_d_om,
            sin_yaw * d_dx_d_vx + cos_yaw * d_dy_d_vx, sin_yaw * d_dx_d_vy + cos_yaw * d_dy_d_vy, sin_yaw * d_dx_d_om + cos_yaw * d_dy_d_om,
            0.0, 0.0, d_dtheta_d_om
        };

        auto mul3 = [](const double a[9], const double b[9], double out[9]) {
        ...
        auto transpose3 = [](const double a[9], double out[9]) {
            for (int row = 0; row < 3; ++row) {
                for (int col = 0; col < 3; ++col) {
        ...
        mul3(j_pose, pose_covariance_.data(), jp);
        transpose3(j_pose, gt);
        mul3(jp, gt, jpjt);
...
void KinematicsEstimator::integratePose(double velocity_x_mps, double velocity_y_mps, double angular_velocity_rad_s, double dt_s) {
    
    // Apply Low-Pass EMA Filter
    // alpha = 1.0 means no filtering (raw data). alpha = 0.1 means heavily smoothed.
    double filtered_velocity_x_mps = (config_.twist_ema_alpha_ * velocity_x_mps)   + ((1.0 - config_.twist_ema_alpha_) * velocity_x_mps_);
    double filtered_velocity_y_mps = (config_.twist_ema_alpha_ * velocity_y_mps)   + ((1.0 - config_.twist_ema_alpha_) * velocity_y_mps_);
    double filtered_omega_rad_s    = (config_.twist_ema_alpha_ * angular_velocity_rad_s) + ((1.0 - config_.twist_ema_alpha_) * velocity_omega_rad_s_);
    ...
    if (std::abs(filtered_omega_rad_s) < config_.se2_integration_omega_threshold_) {
        // Straight-line Euler
        ...
        delta_x_local_m = (filtered_velocity_x_mps * sin_wdt + filtered_velocity_y_mps * (cos_wdt - 1.0)) / filtered_omega_rad_s;
        delta_y_local_m = (filtered_velocity_x_mps * (1.0 - cos_wdt) + filtered_velocity_y_mps * sin_wdt) / filtered_omega_rad_s;
    }
    ...
    // Normalize heading to keep it between -PI and PI (prevents floating point drift)
    orientation_yaw_rad_ = std::atan2(std::sin(orientation_yaw_rad_), std::cos(orientation_yaw_rad_));
...
OdometryEstimate KinematicsEstimator::populateRosMessages(const rclcpp::Time& timestamp) {
    OdometryEstimate estimate;
