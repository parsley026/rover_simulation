...
KinematicsNode::KinematicsNode(const rclcpp::NodeOptions &options,
    ...
    : Node("rover_kinematics", options),
      ...
      config_(), kinematics_solver_(), kinematics_estimator(),
      hardware_interface_(),
  ...
  if (!config_.loadFromFile(config_file_path)) {
    RCLCPP_WARN(this->get_logger(), "Failed to load config from file: %s",
  ...
  clock_sub_ = this->create_subscription<rosgraph_msgs::msg::Clock>(
      "/clock", 10,
      std::bind(&KinematicsNode::clockCallback, this, std::placeholders::_1));
  ...
  cmd_vel_sub_manual_ =
      this->create_subscription<rex_interfaces::msg::RoverControl>(
          "/MQTT/RoverControl", qos,
          std::bind(&KinematicsNode::cmdVelManualCallback, this,
                    std::placeholders::_1));

  wheels_vel_feedback_sub_ =
      this->create_subscription<rex_interfaces::msg::VescStatus>(
          "/CAN/RX/vesc_status", rclcpp::QoS(1000).reliable(),
          std::bind(&KinematicsNode::feedbackCallback, this,
                    std::placeholders::_1));
  ...
  cmd_vel_sub_auto_ = this->create_subscription<geometry_msgs::msg::Twist>(
      config_.cmd_vel_autonomy_topic_, qos,
      std::bind(&KinematicsNode::cmdVelAutoCallback, this,
                std::placeholders::_1));
  ...
  odom_pub_ =
      this->create_publisher<nav_msgs::msg::Odometry>("/kinematics/odom", qos);

  // - - -

  reset_kinematics_srv_ = this->create_service<std_srvs::srv::Empty>(
      "reset_kinematics",
      std::bind(&KinematicsNode::resetKinematicsCallback, this,
                std::placeholders::_1, std::placeholders::_2));

  reset_odometry_srv_ = this->create_service<std_srvs::srv::Empty>(
      "reset_odometry",
      std::bind(&KinematicsNode::resetOdometryCallback, this,
                std::placeholders::_1, std::placeholders::_2));

  RCLCPP_INFO(this->get_logger(),
              "Initialized KinematicsNode: %s with config file: %s",
              this->get_name(), config_file_path.c_str());
}
...
void KinematicsNode::initCmdVelBuffer() {
  ...
  if (config_.use_sim_time_) {
    current_time = rclcpp::Time(sim_time_ns_.load(std::memory_order_relaxed),
                                RCL_ROS_TIME);
  } else {
...
double KinematicsNode::computeWheelQualityScore(
  ...
  const double current_threshold =
      std::max(config_.wheel_quality_high_current_threshold_, 1e-6);
  const double duty_threshold =
      std::clamp(config_.wheel_quality_high_duty_threshold_, 1e-6, 0.999999);

  const double current_penalty =
      std::max(0.0, (current - current_threshold) / current_threshold);
  const double duty_penalty =
      std::max(0.0, (duty_cycle - duty_threshold) / (1.0 - duty_threshold));
...
KinematicsNode::assembleWheelsFromCommand(const WheelCommand &command,
                                          const rclcpp::Time &time) {
  rex_interfaces::msg::Wheels vel_msg;
  vel_msg.header.stamp = time;

  rex_interfaces::msg::Wheel *wheels[4] = {
      &vel_msg.front_left, &vel_msg.front_right, &vel_msg.rear_left,
      &vel_msg.rear_right};

  for (std::size_t i = 0; i < 4; ++i) {
    wheels[i]->drive.command_id = RPM_CONTROL;
    wheels[i]->drive.set_value =
        hardware_interface_.driveSetFromMetersPerSecond(
            command.drive_velocity_mps[i], i);

    wheels[i]->turn.command_id = STEP_MOTOR_CONTROL;
    wheels[i]->turn.set_value = hardware_interface_.steeringSetFromRadians(
        command.steering_angle_rad[i], i);
  }
...
void KinematicsNode::updateWatchdog(const rclcpp::Time &current_time) {
  const bool was_feedback_stale =
      feedback_stale_.load(std::memory_order_acquire);

  const int64_t last_feedback_time_ns =
      last_feedback_time_ns_.load(std::memory_order_acquire);

  const bool feedback_never_received = (last_feedback_time_ns == 0);
  const int64_t elapsed_time_ns =
      current_time.nanoseconds() - last_feedback_time_ns;
  const int64_t timeout_time_ns =
      static_cast<int64_t>(config_.feedback_timeout_sec_ * 1e9);

  const bool is_feedback_stale =
      feedback_never_received || (elapsed_time_ns > timeout_time_ns);
  feedback_stale_.store(is_feedback_stale, std::memory_order_release);

  if (is_feedback_stale && !was_feedback_stale) {
    RCLCPP_ERROR(get_logger(), "Wheel feedback stale for > %.3f sec. Braking.",
                 config_.feedback_timeout_sec_);
    communication_state_.store(COMMUNICATION_STATE_CREATED,
                               std::memory_order_release);
  } else if (!is_feedback_stale && was_feedback_stale) {
    RCLCPP_INFO(get_logger(),
                "Wheel feedback restored. Resuming normal operation.");
    communication_state_.store(COMMUNICATION_STATE_OPENED,
                               std::memory_order_release);
  }
}
...
void KinematicsNode::onUpdate() {
  ...
  if (communication_state_.load(std::memory_order_acquire) ==
      COMMUNICATION_STATE_OPENED) {
    ...
    case CONTROL_MODE_DRIVE:
      ...
      case SYMMETRIC_ACKERMANN_MODE: {
        ...
        // 1. Ghost Velocity: If stationary, use 1.0 m/s to calculate steering
        // geometry
        double calc_linear_m_s =
            (std::fabs(linear_m_s) < 1e-9) ? 1.0 : linear_m_s;

        if (axis_abs > 1e-9) {
          double radius_a_ratio =
              (config_.min_steering_radius_ - config_.max_steering_radius_) /
              0.99;
          double radius_b_ratio =
              (config_.min_steering_radius_ - radius_a_ratio);
        ...
        target_ik = kinematics_solver_.computeSymmetricAckermann(
            calc_linear_m_s, angular_rad_s);
      ...
      case CRAB_MODE: {
        ...
        // 2. Ghost Velocity: Pretend we are moving at 1.0 m/s if throttle is
        // zero
        ...
        // 4. Pass the calculated velocities to the solver to get the correct
        // angles
    ...
    case CONTROL_MODE_DRIVE_AUTONOMY:
      // MPPI Omni provides all 3 degrees of freedom. We bypass multiplexing and
      // solve directly.
      target_ik =
          kinematics_solver_.computeSymmetricAckermann(cmd.vel, cmd.x_axis);
      break;
 ...
 * @return
 */
void KinematicsNode::clockCallback(
...
void KinematicsNode::statusCallback(
  ...
  communication_state_.store(msg->communication_state,
                             std::memory_order_release);
 ...
 * @return
 */
void KinematicsNode::cmdVelAutoCallback(
    const geometry_msgs::msg::Twist::SharedPtr msg) {
  if (control_mode_.load(std::memory_order_acquire) ==
      CONTROL_MODE_DRIVE_AUTONOMY) {
    rex_interfaces::msg::RoverControl local_cmd;

    local_cmd.mode = SYMMETRIC_ACKERMANN_MODE;

    local_cmd.vel = msg->linear.x;
    local_cmd.y_axis = msg->linear.y;
...
void KinematicsNode::feedbackCallback(
  ...
  if (config_.use_sim_time_) {
    receive_time = rclcpp::Time(sim_time_ns_.load(std::memory_order_relaxed),
                                RCL_ROS_TIME);
  } else {
  ...
  if (config_.use_measurement_timestamp_ &&
  ...
  switch (msg->vesc_id) {
  case 0x50:
    current_feedback.front_left.drive.set_value =
        hardware_interface_.metersPerSecondFromErpm(
            msg->erpm,
            HardwareInterface::vescIdToDriveWheelIndex(msg->vesc_id));
    updateWheelQuality(0, *msg);
    break;
  case 0x51:
    current_feedback.front_right.drive.set_value =
        hardware_interface_.metersPerSecondFromErpm(
            msg->erpm,
            HardwareInterface::vescIdToDriveWheelIndex(msg->vesc_id));
    ...
    break;
  case 0x52:
    current_feedback.rear_left.drive.set_value =
        hardware_interface_.metersPerSecondFromErpm(
            msg->erpm,
            HardwareInterface::vescIdToDriveWheelIndex(msg->vesc_id));
    ...
    break;
  case 0x53:
    current_feedback.rear_right.drive.set_value =
        hardware_interface_.metersPerSecondFromErpm(
            msg->erpm,
            HardwareInterface::vescIdToDriveWheelIndex(msg->vesc_id));
    ...
    break;
  case 0x60:
    current_feedback.front_left.turn.set_value =
        hardware_interface_.steeringSetFromPrecisePos(
            msg->precise_pos,
            HardwareInterface::vescIdToTurnWheelIndex(msg->vesc_id));
    updateWheelQuality(0, *msg);
    break;
  case 0x61:
    current_feedback.front_right.turn.set_value =
        hardware_interface_.steeringSetFromPrecisePos(
            msg->precise_pos,
            HardwareInterface::vescIdToTurnWheelIndex(msg->vesc_id));
    updateWheelQuality(1, *msg);
    break;
  case 0x62:
    current_feedback.rear_right.turn.set_value =
        hardware_interface_.steeringSetFromPrecisePos(
            msg->precise_pos,
            HardwareInterface::vescIdToTurnWheelIndex(msg->vesc_id));
    updateWheelQuality(3, *msg);
    break;
  case 0x63:
    current_feedback.rear_left.turn.set_value =
        hardware_interface_.steeringSetFromPrecisePos(
            msg->precise_pos,
            HardwareInterface::vescIdToTurnWheelIndex(msg->vesc_id));
    updateWheelQuality(2, *msg);
  ...
  last_feedback_time_ns_.store(measurement_time.nanoseconds(),
                               std::memory_order_release);
...
void KinematicsNode::resetKinematicsCallback(
    ...
    std::shared_ptr<std_srvs::srv::Empty::Response>) {
  RCLCPP_INFO(this->get_logger(),
              "Restarting Kinematics Node: Reloading config...");
  ...
  if (config_.loadFromFile(config_file_path_)) {
    ...
    timer_->cancel();
    publish_period_ =
        rclcpp::Duration::from_seconds(1.0 / config_.publish_rate_);
    timer_ = this->create_wall_timer(
        publish_period_.to_chrono<std::chrono::milliseconds>(),
        std::bind(&KinematicsNode::onUpdate, this));

    RCLCPP_INFO(this->get_logger(), "succeeded to reload config file: %s",
