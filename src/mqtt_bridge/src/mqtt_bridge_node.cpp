...
public:
  char const *what() { return "JSON assert exception"; }
};
// custom assert(x) for rapidjson library so that it doesn't abort the program
// on errors
#define RAPIDJSON_ASSERT(x)                                                    \
  (static_cast<bool>(x) ? void(0) : throw JsonAssertException())
...
void processMqttWheelsMessage(const char *payloadMsg,
  ...
  if (!ok) {
    ...
    try {
      ...
      // msg.header.stamp =
      // unixMillisecondsToROSTimestamp(d["Timestamp"].GetUint64());

      rth->publishMessage_Wheels(msg);
    } catch (JsonAssertException e) {
      RCLCPP_WARN(node->get_logger(),
                  "JSON assert exception, discarding MQTT message.");
    }
  }
}

void processMqttRoverControlMessage(const char *payloadMsg,
                                    std::shared_ptr<ROSTopicHandler> rth,
                                    std::shared_ptr<rclcpp::Node> node) {
  rapidjson::Document d;
  rapidjson::ParseResult ok = d.Parse(payloadMsg);
...
void processMqttManipulatorControlMessage(const char *payloadMsg,
                                          std::shared_ptr<ROSTopicHandler> rth,
                                          std::shared_ptr<rclcpp::Node> node) {
  rapidjson::Document d;
  rapidjson::ParseResult ok = d.Parse(payloadMsg);
...
void processMqttSamplerControlMessage(const char *payloadMsg,
                                      std::shared_ptr<ROSTopicHandler> rth,
                                      std::shared_ptr<rclcpp::Node> node) {
  rapidjson::Document d;
  rapidjson::ParseResult ok = d.Parse(payloadMsg);
...
// TODO
void processMqttRoverStatusMessage(const char *payloadMsg,
                                   std::shared_ptr<ROSTopicHandler> rth,
                                   std::shared_ptr<rclcpp::Node> node) {
  rapidjson::Document d;
  rapidjson::ParseResult ok = d.Parse(payloadMsg);
...
int main(int argc, char *argv[]) {
  ...
  // if ENABLE_SERVER_CERT_AUTH is true, but file at SSL_CA_PATH cannot be
  // opened, system certificates are used
  ...
  auto SUBSCRIBED_TOPICS_NAMES = mqtt::string_collection::create(
      {"RappTORS/Wheels", "RappTORS/RoverControl",
       "RappTORS/ManipulatorControl", "RappTORS/SamplerControl",
  ...
  node->declare_parameter("mqtt_enable_server_cert_auth",
                          ENABLE_SERVER_CERT_AUTH, param_desc);
  ...
  if (node->get_parameter("mqtt_enable_server_cert_auth").as_bool()) {
    ...
    if (!tstore) {
      RCLCPP_WARN_STREAM(node->get_logger(),
                         "The trust store cannot be opened: "
                             << SSL_CA_PATH
                             << ", not passing it as connection parameter!");
    } else {
      RCLCPP_INFO_STREAM(node->get_logger(),
                         "Trust store found at: " << SSL_CA_PATH);
      tstore_found = true;
    }
  } else {
    RCLCPP_WARN_STREAM(node->get_logger(),
                       "Verification of server certificate is disabled, "
                       "consider adjusting your setup!");
  }

  auto sslopts_builder =
      mqtt::ssl_options_builder()
          .enable_server_cert_auth(
              node->get_parameter("mqtt_enable_server_cert_auth").as_bool())
          .error_handler([/*node*/](const std::string &msg) {
            // RCLCPP_ERROR_STREAM(node->get_logger(), "SSL Error: " << msg);
            std::cout << "SSL Error: " << msg << ", stopping mqtt-bridge!"
                      << std::endl;
            return 1;
          });
  if (tstore_found)
    sslopts_builder = sslopts_builder.trust_store(SSL_CA_PATH);
  auto sslopts = sslopts_builder.finalize();

  std::shared_ptr<mqtt::async_client> cli =
      std::make_shared<mqtt::async_client>(
          node->get_parameter("mqtt_server_address").as_string(),
          node->get_parameter("mqtt_client_id").as_string(),
          mqtt::create_options(MQTT_VERSION));

  RCLCPP_DEBUG_STREAM(
      node->get_logger(),
      "Taking rosparam into account, assuming MQTT broker address: "
          << node->get_parameter("mqtt_server_address").as_string()
          << ", client id: "
          << node->get_parameter("mqtt_client_id").as_string());
  ...
  auto connOpts = mqtt::connect_options_builder()
                      ...
                      .automatic_reconnect(RECONNECT_MIN_RETRY_INTERVAL,
                                           RECONNECT_MAX_RETRY_INTERVAL)
                      .finalize();

  std::shared_ptr<ROSTopicHandler> rth =
      std::make_shared<ROSTopicHandler>(cli, PUBLISHER_QOS, node);

  // callback for connection lost to MQTT broker
  cli->set_connection_lost_handler([node](const std::string &) {
    RCLCPP_ERROR(node->get_logger(),
                 "Connection to MQTT server lost. Trying to reconnect...");
  });
  ...
  cli->set_message_callback([rth, node](mqtt::const_message_ptr mqtt_msg) {
    ...
    RCLCPP_DEBUG(node->get_logger(), "I received (MQTT): [%s] on topic: [%s]",
                 mqtt_msg->get_payload_str().c_str(), messageTopic.c_str());
    ...
    if (messageTopic == "RappTORS/Wheels") {
      ...
      processMqttManipulatorControlMessage(mqtt_msg->get_payload_str().c_str(),
                                           rth, node);
      ...
      RCLCPP_WARN_STREAM(node->get_logger(),
                         "Unknown MQTT topic: "
                             << messageTopic << ", discarding MQTT message.");
    }
  });
    ...
    try {
      ...
      RCLCPP_ERROR_STREAM(node->get_logger(),
                          "Error connecting to MQTT server: "
                              << exc.what() << ", retrying in 5 seconds...");
  ...
  try {
    ...
    std::cout << "Error disconnecting from MQTT server: " << exc.what()
              << std::endl;
    return 1;
  }
