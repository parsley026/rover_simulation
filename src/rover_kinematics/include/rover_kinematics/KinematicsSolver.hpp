...
public:
    ...
    explicit KinematicsSolver(const KinematicsConfig& config = KinematicsConfig{});

    void setConfig(const KinematicsConfig& config);

    // - - -

    WheelCommand computeSymmetricAckermann(double velocity_x_mps, double velocity_omega_rad_s_) const;
    WheelCommand computeFrontOnlyAckermann(double velocity_x_mps, double velocity_omega_rad_s) const;
    WheelCommand computeRearOnlyAckermann( double velocity_x_mps, double velocity_omega_rad_s) const;
